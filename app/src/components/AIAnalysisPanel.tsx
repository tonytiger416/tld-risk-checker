...
const CITATION_BADGE: Record<string, string> = {
  AGB:   'border-[#1e3a7a] text-[#4a7ad5]',
  PREC:  'border-[#3a1e6a] text-[#8a5ad5]',
  ICANN: 'border-[#1a3a5a] text-[#3a8ad5]',
  GAC:   'border-[#4a3a1a] text-[#c88a2a]',
  RFC:   'border-[#1a4a2a] text-[#3ab85a]',
};
...
export function AIAnalysisPanel({ report, cachedText, onCacheUpdate }: Props) {
  ...
  if (!hasApiKey) {
    return (
      <div className="border border-dashed border-[#1e2436] rounded-lg px-5 py-6 text-center">
        <p className="text-sm font-mono text-[#3a4060]">AI analysis not configured</p>
        <p className="text-xs text-[#2a3050] mt-1">
          Add <code className="font-mono text-[#4a5a80]">VITE_CLAUDE_API_KEY</code> in project settings → redeploy.
        </p>
      </div>
    );
  }
      ...
      <div className="bg-[#12151e] border border-[#1e2436] rounded-lg p-4">
        <div className="flex items-center gap-2.5 mb-3">
          <span className="inline-flex gap-1">
            {[0, 150, 300].map(delay => (
              <span key={delay} className="w-1.5 h-1.5 rounded-full bg-[#3a4878] animate-bounce" style={{ animationDelay: `${delay}ms` }} />
            ))}
          </span>
          <span className="text-xs font-mono text-[#3a4878]">Generating analysis for .{report.normalized}</span>
        </div>
        {displayText && (
          <div className="text-xs text-[#4a5270] leading-relaxed whitespace-pre-wrap font-mono bg-[#0b0d14] rounded p-3 min-h-[80px]">
            {displayText}
            <span className="inline-block w-0.5 h-3.5 bg-[#3a4878] ml-0.5 animate-pulse align-text-bottom" />
          </div>
        )}
      </div>
    );
  ...
  if (genState === 'error') {
    return (
      <div className="bg-[#12151e] border border-[#2a1a1a] rounded-lg p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-sm font-mono font-semibold text-[#ff453a] mb-0.5">Analysis failed</p>
            <p className="text-xs text-[#6b7390]">{errorMsg}</p>
          </div>
          <button
            onClick={() => { startedRef.current = false; doGenerate(); }}
            className="flex-shrink-0 text-xs font-mono text-[#4a5270] hover:text-[#8891b0] border border-[#1e2436] hover:border-[#2a3050] px-3 py-1.5 rounded transition-colors"
          >
            Retry
          </button>
        </div>
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Recommendation */}
      <div className="bg-[#12151e] border border-[#1e2436] rounded-lg p-5">
        <div className="flex items-center justify-between mb-4">
          <span className="text-[10px] font-mono font-bold text-[#3a4060] tracking-[0.2em] uppercase">AI Recommendation</span>
          <button
            onClick={() => { startedRef.current = false; doGenerate(); }}
            className="text-[11px] font-mono text-[#3a4060] hover:text-[#6b7390] transition-colors"
            title="Regenerate"
          >
            ↻ regenerate
          </button>
      ...
      {parsed.citations.length > 0 && (
        <div className="bg-[#12151e] border border-[#1e2436] rounded-lg p-5">
          <h3 className="text-[10px] font-mono font-bold text-[#3a4060] tracking-[0.2em] uppercase mb-3">Sources &amp; Precedents</h3>
          <ul className="space-y-2.5">
            {parsed.citations.map((c, i) => {
              const badgeClass = CITATION_BADGE[c.type] ?? 'border-[#1e2436] text-[#4a5270]';
              ...
              return (
                <li key={i} className="flex items-start gap-2.5">
                  ...
                  <span className="text-xs text-[#6b7390] leading-snug">{c.text}</span>
                </li>
              );
            })}
          </ul>
      ...
      {parsed.competitiveLandscape && (
        <div className="bg-[#12151e] border border-[#1e2436] rounded-lg p-5">
          <h3 className="text-[10px] font-mono font-bold text-[#3a4060] tracking-[0.2em] uppercase mb-3">Competitive Landscape</h3>
      ...
      {!parsed.verdict && !parsed.recommendationBody && !parsed.citations.length && !parsed.competitiveLandscape && (
        <div className="bg-[#12151e] border border-[#1e2436] rounded-lg p-5 text-sm text-[#9ba8c0] leading-relaxed whitespace-pre-wrap font-mono">
          {displayText}
        </div>
      )}

      <p className="text-[11px] font-mono text-[#2a3050] px-1">
