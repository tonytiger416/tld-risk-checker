...
const CITATION_BADGE: Record<string, string> = {
  AGB:   'border-[#253d82] text-[#5a80d5]',
  PREC:  'border-[#3e2270] text-[#9060d5]',
  ICANN: 'border-[#1e3e5e] text-[#4a8ad5]',
  GAC:   'border-[#4e3e1e] text-[#c89030]',
  RFC:   'border-[#1e4e2e] text-[#40b860]',
};
...
export function AIAnalysisPanel({ report, cachedText, onCacheUpdate }: Props) {
  ...
  if (!hasApiKey) {
    return (
      <div className="border border-dashed border-[#21293d] rounded-lg px-5 py-6 text-center">
        <p className="text-sm font-mono text-[#4a566e]">AI analysis not configured</p>
        <p className="text-xs text-[#3a4560] mt-1">
          Add <code className="font-mono text-[#5a7aaa]">VITE_CLAUDE_API_KEY</code> in project settings → redeploy.
        </p>
      </div>
    );
  }
      ...
      <div className="bg-[#161b28] border border-[#21293d] rounded-lg p-4">
        <div className="flex items-center gap-2.5 mb-3">
          <span className="inline-flex gap-1">
            {[0, 150, 300].map(delay => (
              <span key={delay} className="w-1.5 h-1.5 rounded-full bg-[#4a5e88] animate-bounce" style={{ animationDelay: `${delay}ms` }} />
            ))}
          </span>
  ...
  if (genState === 'error') {
    return (
      <div className="bg-[#161b28] border border-[#2e1a1a] rounded-lg p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            className="flex-shrink-0 text-xs font-mono text-[#5c698a] hover:text-[#8a95b5] border border-[#21293d] hover:border-[#2e3a52] px-3 py-1.5 rounded transition-colors"
          >
            Retry
          </button>
        </div>
    ...
    <div className="space-y-3">
      ...
      <div className="bg-[#161b28] border border-[#21293d] rounded-lg p-5">
        <div className="flex items-center justify-between mb-4">
          <span className="text-[10px] font-mono font-bold text-[#5c698a] tracking-[0.2em] uppercase">AI Recommendation</span>
          <button
            onClick={() => { startedRef.current = false; doGenerate(); }}
            className="text-[11px] font-mono text-[#4a566e] hover:text-[#6b7898] transition-colors"
            title="Regenerate"
          >
            ↻ regenerate
          </button>
      ...
      {parsed.citations.length > 0 && (
        <div className="bg-[#161b28] border border-[#21293d] rounded-lg p-5">
          <h3 className="text-[10px] font-mono font-bold text-[#5c698a] tracking-[0.2em] uppercase mb-3">Sources &amp; Precedents</h3>
          <ul className="space-y-2.5">
            {parsed.citations.map((c, i) => {
              const badgeClass = CITATION_BADGE[c.type] ?? 'border-[#21293d] text-[#5c698a]';
      ...
      {!parsed.verdict && !parsed.recommendationBody && !parsed.citations.length && !parsed.competitiveLandscape && (
        <div className="bg-[#161b28] border border-[#21293d] rounded-lg p-5 text-sm text-[#9ba8c0] leading-relaxed whitespace-pre-wrap font-mono">
          {displayText}
        </div>
      )}

      <p className="text-[11px] font-mono text-[#3a4560] px-1">
